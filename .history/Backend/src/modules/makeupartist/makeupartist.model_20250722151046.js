...
MakeupArtist.prototype.addPortfolioImages = function (imageUrls) {
  if (!Array.isArray(imageUrls)) return;
  const uniqueUrls = imageUrls.filter(url => typeof url === 'string' && url && !this.portfolioImages.includes(url));
  if (uniqueUrls.length > 0) {
    this.portfolioImages = [...this.portfolioImages, ...uniqueUrls];
    this.changed('portfolioImages', true);
  }
};
