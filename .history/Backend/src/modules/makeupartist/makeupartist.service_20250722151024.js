...
class MakeupArtistService {
  ...
  async addPortfolioImages(makeupArtistId, files) {
    try {
      const makeupArtist = await MakeupArtist.findByPk(makeupArtistId);
      if (!makeupArtist) {
        throw {
      ...
      const uploadPromises = files.map(file => 
        cloudinarySvc.fileUpload(file.path, "makeupartists/portfolio/")
      );
      const uploadResults = await Promise.all(uploadPromises);
      const imageUrls = uploadResults.map(result => result.url);

      makeupArtist.addPortfolioImages(imageUrls);
      await makeupArtist.save();
      await makeupArtist.reload();
